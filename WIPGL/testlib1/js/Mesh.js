...
class Mesh {
  ...
  draw(){
    ...
    this.ctx.vertexAttribPointer(
  		...
  		this.ctx.FLOAT,
  		this.ctx.FALSE,
  		6 * Float32Array.BYTES_PER_ELEMENT, // taille d'un element
  		0 // décalage
  	);

    this.ctx.vertexAttribPointer(
  		Shader.NORMALE, //attribLocation
  		3, // nb d'elements par attr
  		this.ctx.FLOAT,
  		this.ctx.FALSE,
  		6 * Float32Array.BYTES_PER_ELEMENT, // taille d'un element
    ...
    this.ctx.drawElements(/*this.ctx.LINES*/this.ctx.TRIANGLES, this.nbIndex, this.ctx.UNSIGNED_SHORT, 0);
  }
}

/*
class IndexedMesh extends Mesh {
  constructor(display, verts, indexes) {
    super(display, data, null);

    this.indexBuffer = this.ctx.createBuffer();
  	this.ctx.bindBuffer(this.ctx.ARRAY_BUFFER, this.indexBuffer);
